  ...
  return (
    <div className="space-y-2">
      {Object.entries(byRace).map(([race, counts]) => {
        ...
        const nPct = counts.total ? Math.round((counts.neutral / counts.total) * 100) : 0;
        const rPct = counts.total ? Math.round((counts.reject / counts.total) * 100) : 0;
        return (
          <div key={race} className="flex items-center gap-3 text-sm">
            <span className="w-16 text-slate-400 text-xs truncate">{race}</span>
            <div className="flex-1 h-3 bg-slate-800 rounded-full overflow-hidden flex">
              <div className="bg-emerald-500 h-full transition-all" style={{ width: `${sPct}%` }} />
              <div className="bg-amber-500 h-full transition-all" style={{ width: `${nPct}%` }} />
              <div className="bg-red-500 h-full transition-all" style={{ width: `${rPct}%` }} />
            </div>
