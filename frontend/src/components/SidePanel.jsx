...
export default function SidePanel({ provisions, selectedGrc, grcSentiment, votePrediction, onLeverChange, className }) {
  ...
  return (
    <div className={`${className} border-l border-slate-800 overflow-y-auto p-4 space-y-4`}>
      {/* Policy Summary */}
      <div>
        <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Policy Provisions</h3>
      ...
      {grcData && (
        <>
          <div>
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{selectedGrc}</h3>
